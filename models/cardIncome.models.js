...
module.exports.fetchCardIncomeByUserId = (userid, time, category)=>{
    ...
    let queryStr = `SELECT * FROM cardTransaction WHERE user_id = $1 AND transaction_type = $2`
...
module.exports.fetchTotalCardIncomeByUserId = (userid, time, grouped)=>{
    const timeAllowedQuery = ['today', 'week', '10days', 'month', 'year'];
    if(time){
        if(!timeAllowedQuery.includes(time)){
            return Promise.reject({status: 400, msg: "Bad request"})
    ...
    queryStr = `SELECT ${placeholder} SUM(amountPence) AS total_cardincome  FROM cardTransaction WHERE user_id = $1 AND transaction_type = $2 `
