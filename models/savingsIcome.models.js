...
module.exports.fetchSavingsIncomeByUserId = (userid, time, category) =>{
    ...
    let queryStr = `SELECT * FROM savingsTransaction WHERE user_id = $1 AND transaction_type = $2`
...
module.exports.fetchTotalSavingsIncomeByUserId = (userid, time, grouped) =>{
    const timeAllowedQuery = ['today', 'week', '10days', 'month', 'year'];
    if(time){
        if(!timeAllowedQuery.includes(time)){
            return Promise.reject({status: 400, msg: "Bad request"})
    ...
    queryStr = `SELECT ${placeholder} SUM(amountPence) AS total_savingsincome  FROM savingsTransaction WHERE user_id = $1 AND transaction_type = $2 `
