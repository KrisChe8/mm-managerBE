...
module.exports.fetchCashIncomeByUserId = (userid, time, category)=>{
    ...
    let queryStr = `SELECT * FROM cashTransaction WHERE user_id = $1 AND transaction_type = $2`
    if(category){
...
module.exports.fetchTotalCashIncomeByUserId = (userid, time, grouped) =>{
    const timeAllowedQuery = ['today', 'week', '10days', 'month', 'year'];
    if(time){
        if(!timeAllowedQuery.includes(time)){
            return Promise.reject({status: 400, msg: "Bad request"})
    ...
    queryStr = `SELECT ${placeholder} SUM(amountPence) AS total_cashincome  FROM cashTransaction WHERE user_id = $1 AND transaction_type = $2 `
